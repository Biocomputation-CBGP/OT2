...
"""
"""
ESTA ES LA NUEVA VERSION QUE ES CON EL TERMOCICLADOR INTEGRADO
Lo que se va a hacer es intentar tener otra variables que sea el protocolo con termociclador en el cual solo se podra tener una placa final que estara integrada en el termociclador
Eso es lo unico que va a cambiar, lo demas sera igual.
De esta manera, si queremos hacer mas placas sera un False en el termociclador y si lo queremos sera un True
"""
...
# ----------------------------------
    ...
    class setted_parameters():
        ...
        def __init__(self, modular_parameters):
            ...
            ### New variables related to the thermocycler
            self.tc_module = None
            if modular_parameters["Presence of thermocycler"]["Value"].lower() == "true":
                self.thermocycler = True
                self.name_thermocycler = 'thermocycler'
                self.thermocycler_profile_file = "/data/user_storage/"+modular_parameters["PCR Program File"]["Value"]+".csv"
                if modular_parameters["Final block temperature"]["Value"] == "-":
                    self.final_block_temperature = None
                else:
                    self.final_block_temperature = int(modular_parameters["Final block temperature"]["Value"])
                if modular_parameters["Final Lid Open"]["Value"].lower() == "false":
                    self.final_open_lid = False
                elif modular_parameters["Final Lid Open"]["Value"].lower() == "true":
                    self.final_open_lid = True
                else:
                    raise Exception("Pause for final lid open variable can only be True or False")
                #print(modular_parameters["Pause for putting caps"]["Value"].lower())
                if modular_parameters["Pause for putting caps"]["Value"].lower() == "false":
                    self.pause_caps = False
                elif modular_parameters["Pause for putting caps"]["Value"].lower() == "true":
                    self.pause_caps = True
                else:
                    raise Exception("Final Lid Open value can only be True or False")
            elif modular_parameters["Presence of thermocycler"]["Value"].lower() == "false":
                self.thermocycler = False
                self.name_thermocycler = None
                self.thermocycler_profile_file = None
                self.final_block_temperature = None
                self.final_open_lid = None
                self.pause_caps = None
            else:
                raise Exception("Replace Tiprack value can only be True or False")
            ###
...
# ----------------------------------
    ...
    def check_setted_parameters(variables):
        ...
        #-------------------------------------------------------------------------++++
        """
        Vamos a chekcear que las cosas del termociclador esten bien solo cuando el termocycler este a true, si no, damos que todo lo demas esta bien y ya esta
        no vamos a darle la oportunidad de fastidiarla mas a la persona
        """
        if variables.thermocycler: #We are going to use the thermocycler
            # The program file exists
            if os.path.isfile(variables.thermocycler_profile_file) == False:
                errors.append(f"The program file '{variables.thermocycler_profile_file}' does not exist or it is not found!")
                return errors, warnings
            
            # temperatures need to be inside of thermocycler ranges
            # temperatures of the profile column
            try:
                if any(temperature < 4 or temperature > 99 for temperature in pd.read_csv(variables.thermocycler_profile_file)["Temperature (C)"].values):
                    errors.append("There is a temperature of the block in the program file that is either inferior to 4 degrees or superior to 99, which is not posisble!")
            except:
                raise Exception(f"Something went wrong when trying to read PCR program file '{variables.thermocycler_profile_file}', make sure it is a csv and respects the format needed")
            # temperature of the block at the end of the PCR
            if variables.final_block_temperature != None and (variables.final_block_temperature < 4 or variables.final_block_temperature > 99):
                errors.append("Final block temperature cant be reached with the thermocycler" )
            # temperature of the lid --> this is going to be a warning, not an error
            if max(pd.read_csv(variables.thermocycler_profile_file)["Temperature (C)"].values) + 10 > 110:
                warnings.append("The lid temperature in a part of the PCR program cant be reached, it will be defined as 110 degrees")
        else: #We are not going to use the thermocycler so ther eis no need to test the variables
            pass
    ...
    def setting_labware(variables):
        ...
        if variables.thermocycler:
            if number_plates_pcr > 1:
                raise Exception("The number of final plates needed for this protocol is higher than 1 and it can't be done with a thermocyler")
            else:
    ...
    def run_program_PCR(tc_mod, program, final_lid_state, final_block_state, volume_sample):
        ...
        lid_temperature = max(program["Temperature (C)"].values)+10
...
# We use a try and except so we can give some more user-friendly infromation about the errors that we can receive
    try:
        ...
        #----------------------------------------------------------------------------------------------------------
        # Lets perform the pcr in case that it is required
        if variables.thermocycler:
